...
public class ShaheedPakagePotraitAdapter extends RecyclerView.Adapter<ShaheedPakagePotraitAdapter.MyViewHolder> {
    private Context mContext;
    private List<ShaheedPakageModel.Val> mList;
    public ShaheedPakagePotraitAdapter(Context context, List<ShaheedPakageModel.Val> list) {
    ...
    public MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.shaheed_pakage_item_layout2, parent, false);
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
        ...
        holder.relation_tv.setText("Relation: "+mList.get(position).getRelations());
        holder.pkg_type_tv.setText("Pakage: "+mList.get(position).getPackageName());
        holder.details_tv.setText(mList.get(position).getPackageDetails());
        if (mList.get(position).getPackageStatus().equals("1")){
            holder.status_tv.setText("Awarded: "+"Yes");
        }else {
            holder.status_tv.setText("Awarded: "+"No");
        }
